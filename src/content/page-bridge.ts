(() => {
  if ((window as any).__ytDualSubBridgeLoaded) {
    return;
  }

  (window as any).__ytDualSubBridgeLoaded = true;
  ...
  function readSimpleOrRunText(value: any): string {
    if (!value) return "";
    if (typeof value.simpleText === "string") return value.simpleText;

    if (Array.isArray(value.runs)) {
      return value.runs.map((item: any) => item?.text ?? "").join("").trim();
    }
  ...
  function emitState(state: BridgeState): void {
    ...
    console.log("[YT Dual Sub][bridge] emitState", {
      videoId: state.videoId,
      captionsEnabled: state.captionsEnabled,
      selectedTrack: state.selectedTrack,
      captionTracksCount: state.captionTracks.length,
      captionTracks: state.captionTracks,
      translationLanguagesCount: state.translationLanguages.length,
  ...
  function extractPlayerResponseFromScripts(): any | null {
    ...
    for (const script of scripts) {
      ...
      try {
        ...
        console.warn(
          `${LOG_PREFIX} failed to parse ytInitialPlayerResponse from script`,
        );
        continue;
      }
    }
  ...
  function getPlayerResponse(): any | null {
    ...
    try {
      if (player?.getPlayerResponse) {
        ...
        console.log(
          `${LOG_PREFIX} player response via movie_player.getPlayerResponse`,
        );
        return response;
      }
    } catch (error) {
    ...
    if (playerWindow.ytInitialPlayerResponse) {
      console.log(
        `${LOG_PREFIX} player response via window.ytInitialPlayerResponse`,
      );
  ...
  function mapTranslationLanguages(playerResponse: any): Array<{
    languageCode: string;
    languageName: string;
  }> {
    const raw =
    ...
    const mapped = raw
      .filter((item: any) => typeof item?.languageCode === "string")
      .map((item: any) => ({
        languageCode: item.languageCode,
        languageName:
          readSimpleOrRunText(item.languageName) || item.languageCode,
      }));

    console.log(`${LOG_PREFIX} mapTranslationLanguages`, {
      raw: raw.length,
  ...
  function mapCaptionTracks(playerResponse: any): PageBridgeTrack[] {
    ...
    const mapped = tracks
      ...
      .map((track: any) => ({
        ...
        languageName:
          readSimpleOrRunText(track.name) || track.languageCode || "",
        vssId: track.vssId,
        kind: track.kind,
    ...
    console.log(`${LOG_PREFIX} mapCaptionTracks`, {
      raw: tracks.length,
      mapped: mapped.length,
      languages: mapped.map((track: PageBridgeTrack) => track.languageCode),
    });

    return mapped;
  }

  function normalizeTrackCandidate(
    track: any,
  ): Partial<PageBridgeTrack> | null {
    ...
    return {
      ...
      languageCode: track.languageCode ?? "",
      languageName:
        track.displayName ?? track.name?.simpleText ?? track.languageCode ?? "",
      vssId: track.vssId,
      kind: track.kind,
  ...
  function findSelectedTrack(
    ...
    try {
      ...
      if (currentTrack) {
        ...
        console.warn(
          `${LOG_PREFIX} current track not matched to captionTracks`,
          {
            currentTrack,
  ...
  function buildState(): BridgeState {
    ...
    const translationLanguages = mapTranslationLanguages(playerResponse);
    ...
    const state = {
      ...
      captionsEnabled: areCaptionsEnabled(player) || tracks.length > 0,
      selectedTrack,
    ...
    if (!state.videoId) {
      console.log(`${LOG_PREFIX} buildState without videoId`, {
        url: state.url,
      });
    }
